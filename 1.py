...
def call_func(command, arguments = tuple()):
    config  = load_config()
    try:
        with psycopg2.connect(**config) as conn:
            with conn.cursor() as cur:

                if command == 'get_by_phone_prefix':
                    cur.callproc(command, arguments)
                    rows = cur.fetchall()
                    print("The number of parts: ", cur.rowcount)
                    print("--------------------------------------------")
                    for row in rows:
...
while True:
    ...
    if a[0] == "getphone":
        get(name = a[1])
    if a[0] == "getnames":
        call_func('get_by_phone_prefix', (a[1], ))
    if a[0] == "add":
        call_func(f"CALL insert_update('{a[1]}', '{str(a[2])}')")
        print(f"ADDED  {a[1]}, {a[2]}")
    if a[0] == "insert_many":
        n = int(a[1])
        print("name   phone")
        names = ""
        ...
        for i in range(n):
            ...
            #names.append(name)
            #phones.append(phone)
        call_func("CALL insert_many('{" + names + "}', '{" + phones + "}')")
